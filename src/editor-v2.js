...
class AstraeonEditor{
  constructor(){
    ...
    this.undoStack=[]; this.redoStack=[]; this.history=[]; this.dirty=false;
    this.world=W.generateWorld({seed:this.design.seed,custom:this.design});
    this.cacheUI(); this.buildPalette(); this.bind(); this.resize(); this.fitWorld(); this.refreshAll(); this.draw();
  }

  cacheUI(){
    this.ui={seed:$('#seedInput'),object:$('#objectSelect'),mob:$('#mobSelect'),brush:$('#brushSize'),coords:$('#cursorCoord'),zoom:$('#zoomLabel'),inspector:$('#inspector'),stats:$('#stats'),history:$('#historyList'),saveState:$('#saveState'),notice:$('#notice'),jsonDialog:$('#jsonDialog'),jsonText:$('#jsonText'),statusTool:$('#statusTool'),statusSeed:$('#statusSeed')};
  }
  ...
  bind(){
    ...
    $('#playBtn').addEventListener('click',()=>{this.save();location.href='index.html';});
    ...
    $('#exportBtn').addEventListener('click',()=>this.openJson(false)); $('#importBtn').addEventListener('click',()=>this.openJson(true));
    $('#jsonClose').addEventListener('click',()=>this.ui.jsonDialog.classList.add('hidden')); $('#jsonApply').addEventListener('click',()=>this.applyJson());
    $('#zoomIn').addEventListener('click',()=>this.zoomAt(1.18,this.canvas.clientWidth/2,this.canvas.clientHeight/2));
    $('#zoomOut').addEventListener('click',()=>this.zoomAt(.84,this.canvas.clientWidth/2,this.canvas.clientHeight/2)); $('#zoomFit').addEventListener('click',()=>this.fitWorld());
    $$('.layer-toggle').forEach(ch=>ch.addEventListener('change',()=>{this.layers[ch.dataset.layer]=ch.checked;this.draw();}));
    this.canvas.addEventListener('contextmenu',e=>e.preventDefault());
    this.canvas.addEventListener('pointerdown',e=>this.pointerDown(e)); this.canvas.addEventListener('pointermove',e=>this.pointerMove(e)); window.addEventListener('pointerup',()=>this.pointerUp());
    this.canvas.addEventListener('wheel',e=>this.wheel(e),{passive:false}); window.addEventListener('keydown',e=>this.keydown(e));
  }

  setTool(tool){
    this.tool=tool; $$('.tool-btn').forEach(b=>b.classList.toggle('active',b.dataset.tool===tool));
    this.ui.statusTool.textContent=this.toolName(tool); this.canvas.style.cursor=tool==='pan'?'grab':tool==='select'?'default':'crosshair';
  }

  resize(){
    const r=this.canvas.getBoundingClientRect(); this.canvas.width=Math.max(1,Math.floor(r.width*this.dpr)); this.canvas.height=Math.max(1,Math.floor(r.height*this.dpr));
  ...
  fitWorld(){
    ...
    const z=Math.min((this.width-60)/worldPx,(this.height-60)/worldPy); this.view.zoom=W.clamp(z,.12,1.8);
    this.view.x=(this.width-worldPx*this.view.zoom)/2; this.view.y=(this.height-worldPy*this.view.zoom)/2; this.refreshZoom(); this.draw();
  }

  zoomAt(factor,sx,sy){
    const old=this.view.zoom, next=W.clamp(old*factor,.12,2.2); const wx=(sx-this.view.x)/old, wy=(sy-this.view.y)/old;
    this.view.zoom=next; this.view.x=sx-wx*next; this.view.y=sy-wy*next; this.refreshZoom(); this.draw();
  }
  wheel(e){e.preventDefault();this.zoomAt(e.deltaY<0?1.12:.89,e.offsetX,e.offsetY);}
  refreshZoom(){this.ui.zoom.textContent=`${Math.round(this.view.zoom*100)}%`;}

  screenToTile(sx,sy){
    const wx=(sx-this.view.x)/this.view.zoom, wy=(sy-this.view.y)/this.view.zoom;
    return {x:Math.floor(wx/W.TILE),y:Math.floor(wy/W.TILE)};
  }
  ...
  pointerMove(e){
    ...
    this.pointer.x=sx;this.pointer.y=sy;this.pointer.tx=t.x;this.pointer.ty=t.y; this.ui.coords.textContent=`tile ${t.x}, ${t.y}`;
    ...
    if(this.pointer.down&&['biome','road','water','object','collision','erase'].includes(this.tool)) this.applyAtPointer(sx,sy,false);
    else this.draw();
  }
  pointerUp(){
    if(!this.pointer.down)return; this.pointer.down=false; this.dragPan=false; this.canvas.style.cursor=this.tool==='pan'?'grab':this.tool==='select'?'default':'crosshair';
    if(this.strokeSnapshot&&this.dirty){this.undoStack.push(this.strokeSnapshot);if(this.undoStack.length>40)this.undoStack.shift();this.redoStack=[];this.strokeSnapshot=null;}
  }
  ...
  applyAtPointer(sx,sy,rebuild=true){
    ...
    for(let y=t.y-radius;y<=t.y+radius;y++)for(let x=t.x-radius;x<=t.x+radius;x++){
      ...
      if(Math.hypot(x-t.x,y-t.y)>radius+.25&&radius>0)continue; this.applyTile(x,y);
    }
  ...
  applySpawn(x,y){
    this.design.spawns=this.design.spawns.filter(s=>s.x!==x||s.y!==y); this.design.spawns.push({x,y,type:this.mob}); this.selected={x,y};
  }

  snapshot(){return JSON.stringify({overrides:this.design.overrides,spawns:this.design.spawns,seed:this.design.seed});}
  restoreSnapshot(raw){
    const s=JSON.parse(raw); this.design.overrides=s.overrides||{};this.design.spawns=s.spawns||[];this.design.seed=s.seed||this.design.seed;this.ui.seed.value=this.design.seed;this.rebuild();this.markDirty();this.refreshAll();
  }
  commitEdit(label){this.rebuild();this.markDirty();this.addHistory(label);this.refreshAll();}
  markDirty(){this.dirty=true;this.ui.saveState.textContent='Alterações não salvas';this.ui.saveState.style.color='#f0c76c';}
  ...
  regenerateSeed(){
    const next=(this.ui.seed.value||'ASTRAEON-2').trim();if(next===this.design.seed)return;this.undoStack.push(this.snapshot());this.design.seed=next;this.redoStack=[];this.commitEdit('Semente alterada');this.notify('Mundo procedural regenerado.');
  }
  newMap(){
  ...
  save(){
    this.design.version=W.VERSION;this.design.width=this.world.width;this.design.height=this.world.height;W.saveWorldDesign(this.design);this.dirty=false;this.ui.saveState.textContent='Mapa salvo no navegador';this.ui.saveState.style.color='#70ce90';this.addHistory('Mapa salvo');this.notify('Mapa 2.0 salvo. O jogo já usará estas alterações.');
  }

  openJson(importMode){
    this.importMode=importMode;this.ui.jsonDialog.classList.remove('hidden');this.ui.jsonText.value=importMode?'':JSON.stringify(this.design,null,2);$('#jsonTitle').textContent=importMode?'Importar mapa JSON':'Exportar mapa JSON';$('#jsonHelp').textContent=importMode?'Cole um mapa Astraeon 2.0 e clique em Aplicar.':'Copie este JSON para versionar, compartilhar ou arquivar o mapa.';$('#jsonApply').textContent=importMode?'Aplicar importação':'Copiar JSON';
  }
  async applyJson(){
    ...
    try{const parsed=JSON.parse(this.ui.jsonText.value);if(!parsed||typeof parsed!=='object'||!parsed.seed)throw new Error();this.undoStack.push(this.snapshot());this.design=Object.assign(W.makeDefaultDesign(parsed.seed),parsed);this.design.overrides=this.design.overrides||{};this.design.spawns=Array.isArray(this.design.spawns)?this.design.spawns:[];this.ui.seed.value=this.design.seed;this.rebuild();this.markDirty();this.fitWorld();this.refreshAll();this.ui.jsonDialog.classList.add('hidden');this.addHistory('Mapa importado');this.notify('Mapa importado com sucesso.');}catch(_){this.notify('JSON inválido ou incompatível.');}
  }
  ...
  refreshAll(){this.refreshStats();this.refreshInspector();this.refreshHistory();this.refreshZoom();this.ui.statusSeed.textContent=this.design.seed;this.draw();}
  refreshStats(){
    ...
    this.ui.stats.innerHTML=`<div class="stat"><b>${overrides}</b><small>tiles editados</small></div><div class="stat"><b>${spawns}</b><small>spawns</small></div><div class="stat"><b>${blocked}</b><small>bloqueios</small></div><div class="stat"><b>5</b><small>biomas</small></div>`;
  }
  refreshInspector(){
    if(!this.selected){this.ui.inspector.innerHTML='<h3>Inspetor</h3><p class="hint">Use a ferramenta Seleção e clique em um tile para examinar terreno, objeto, colisão e spawn.</p>';return;}
    const t=this.world.get(this.selected.x,this.selected.y),b=t?W.BIOMES[t.biome]:null,spawn=this.design.spawns.find(s=>s.x===this.selected.x&&s.y===this.selected.y);
    if(!t)return;
    this.ui.inspector.innerHTML=`<h3>${b.icon} ${b.name}</h3><div class="kv"><span>Coordenada</span><b>${t.x}, ${t.y}</b></div><div class="kv"><span>Clima</span><b>${b.climate}</b></div><div class="kv"><span>Terreno</span><b>${t.kind}</b></div><div class="kv"><span>Objeto</span><b>${t.object?this.objectName(t.object):'—'}</b></div><div class="kv"><span>Colisão</span><b>${t.blocked?'Bloqueado':'Livre'}</b></div><div class="kv"><span>Spawn</span><b>${spawn?spawn.type:'—'}</b></div><p class="hint">${b.feature}</p>`;
  }
  addHistory(label){this.history.unshift({label,time:new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})});this.history=this.history.slice(0,10);this.refreshHistory();}
  refreshHistory(){this.ui.history.innerHTML=this.history.length?this.history.map(h=>`<div class="history-item"><b>${h.time}</b> · ${h.label}</div>`).join(''):'<p class="hint">As ações desta sessão aparecerão aqui.</p>';}
  notify(text){this.ui.notice.textContent=text;this.ui.notice.classList.add('show');clearTimeout(this.noticeTimer);this.noticeTimer=setTimeout(()=>this.ui.notice.classList.remove('show'),2100);}
  ...
  draw(){
    ...
    for(let y=sy;y<ey;y++)for(let x=sx;x<ex;x++){
      ...
      ctx.fillStyle=color;ctx.fillRect(px,py,Math.ceil(ts)+1,Math.ceil(ts)+1);
